...
def generate_password(
    ...
    if no_repeats and length > len(char_pool):
        print(
            "Not enough unique characters available to generate a password of this length without repeats."
        )
        return None
...
def main():
    ...
    while True:
        option = input(
            "Would you like to generate a (p)assword or a (ph)rase? (p/ph): "
        ).lower()
        ...
        if option == "p":
            ...
            while True:
                try:
                    length = int(
                        input(
                            "Input the length of the password (between 5 and 64): "
                        )
                    )
                    if 5 <= length <= 64:
                        break
                        ...
                        print(
                            "Please enter a number between 5 and 64."
                        )
                except ValueError:
                    print(
            ...
            while True:
                ...
                no_repeats = (
                    input(
                        "Do you want to generate a password with no repeating characters? (y/n): "
                    ).lower()
                    == "y"
                )
                ...
                if password:
                    ...
                    print(
                        "You must select at least one option to create a password and ensure there are enough unique characters if no repeats are selected. Please try again."
                    )
            break
            ...
            while True:
                try:
                    num_words = int(
                        input(
                            "Input the number of words for the passphrase (between 3 and 15): "
                        )
                    )
                    if 3 <= num_words <= 15:
                        break
            ...
            capitalize_first = (
                input(
                    "Do you want to capitalize the first letter of each word? (y/n): "
                ).lower()
                == "y"
            )
            capitalize_all = (
                input(
                    "Do you want to capitalize all letters? (y/n): "
                ).lower()
                == "y"
            )
            word_separator = (
                input(
                    "Enter a single character to use as a word separator (default is space): "
                )
                or " "
            )
            add_numbers = (
                input(
                    "Do you want to add a number to each word? (y/n): "
                ).lower()
                == "y"
            )
            ...
            if passphrase:
                ...
                print(
                    "Could not generate a passphrase with the given parameters. Please try again."
                )
            break

        else:
            print(
                "Invalid option. Please choose 'p' for password or 'ph' for passphrase."
            )
